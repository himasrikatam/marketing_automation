...
if __name__ == "__main__":
    ...
    inputs = {
        "product_name": "Cold Emailing Automation Tool",
        "target_audience": "Small and Medium Enterprises (SMEs)",
        "product_description": "A tool that automates cold emailing in Excel using AI, saving time and reducing errors.",
        "budget": "Rs. 50,000",
        "current_date": datetime.now().strftime("%Y-%m-%d"),
    }
    
    with langfuse.start_as_current_span(name="marketing_automation") as span:
        # span.update_trace(session_id = st.session_state.get("session_id", "testing_session")) 
        span.update(input={"inputs": inputs})
        span.update_trace(user_id="himasri")
